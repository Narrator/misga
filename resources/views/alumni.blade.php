        ...
        <div class="content container">
            <div class="page-wrapper">
                <header class="page-heading clearfix">
                    <h1 class="heading-title pull-left">Alumni</h1>
                            ...
                            @foreach($alumni as $alum)
                                <div class="row page-row" >
                                    <figure class="thumb col-md-3 col-sm-4 col-xs-6">
                                        <img class="img-responsive" src="{{ $alum->picture }}" alt="" />
                                    </figure>
                                    <div class="details col-md-9 col-sm-8 col-xs-6">
                                        <h3 class="title">{{ $alum->name }}</h3>
                                        <h4>{{ $alum->title }}</h4>
                                        {{ str_limit(strip_tags($alum->bio), $limit = 300, $end = '...') }}
                                        <br><br>
                                        <a class="btn btn-theme read-more" href="/user-profile/{{ $alum->id }}">Read more<i class="fa fa-chevron-right"></i></a>
                                        @if (Auth::user() && (Auth::user()->isAdmin() || Auth::user()->id == $alum->id ))
                                            <a class="btn btn-warning red read-more" href="/edit-user/{{ $alum->id }}">Edit Profile<i class="fa fa-chevron-right"></i></a>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
