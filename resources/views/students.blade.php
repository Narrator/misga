                            ...
                            @foreach($students as $student)
                                <div class="row page-row" >
                                    <figure class="thumb col-md-3 col-sm-4 col-xs-6">
                                        <img class="img-responsive" src="{{ $student->picture }}" alt="" />
                                    </figure>
                                    <div class="details col-md-9 col-sm-8 col-xs-6">
                                        <h3 class="title">{{ $student->name }}</h3>
                                        <h4>{{ $student->title }}</h4>
                                        {{ str_limit(strip_tags($student->bio), $limit = 300, $end = '...') }}
                                        <br><br>
                                        <a class="btn btn-theme read-more" href="/user-profile/{{ $student->id }}">Read more<i class="fa fa-chevron-right"></i></a>
                                        @if (Auth::user() && (Auth::user()->isAdmin() || Auth::user()->id == $student->id ))
                                            <a class="btn btn-warning red read-more" href="/edit-user/{{ $student->id }}">Edit Profile<i class="fa fa-chevron-right"></i></a>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
